...
**Status:** Audit Complete | Hardening in Progress | Empirical Re-Audit 2026-03-01
...
## Empirical Re-Audit Findings (2026-03-01)

### New Issues Found

| # | Severity | Finding | File | Status |
|---|----------|---------|------|--------|
| SEC-1 | **P0** | Inline `onerror` handler violates CSP | `ecosystem-map.html:1286` | Open |
| SEC-2 | **P1** | Inline `<script type="module">` in build.html | `build.html:825-855` | Open |
| SEC-3 | **P1** | `scriptSrcAttr: unsafe-inline` too broad | `server.cjs:131` | Open |
| SEC-4 | **P1** | Redis endpoint unprotected in dev mode | `server.cjs:307-361` | Open |
| SEC-5 | **P1** | Missing SRI on D3.js CDN | `ecosystem-map.html:1283` | Open |
| SEC-6 | **P3** | parseInt pagination no lower bound | `admin.js:323,659` | Open |

### Confirmed Strengths (Empirically Verified)

- Timing-safe Redis API key comparison (SHA-256 + crypto.timingSafeEqual) ✓
- All PostgreSQL queries use parameterized `$1, $2` placeholders ✓
- Cookie flags: httpOnly + Secure + SameSite correct ✓
- Rate limiting: 400 req/15min per IP, health check exempted ✓
- CORS: production requires explicit ALLOWED_ORIGINS, fails to start otherwise ✓
- Error sanitization: no stack traces in production ✓
- Static file blocking: .env, .json, .md blocked from serving ✓
- Helmet: CSP + HSTS (1 year) + X-Content-Type-Options enforced ✓
- npm audit: 0 vulnerabilities ✓

### Risk Assessment Update

**Overall Security Score: B+ (unchanged)**
- Strengths are solid and empirically verified
- New findings are edge cases (CSP violations, dev-mode gaps)
- No structural flaws or logic vulnerabilities found
