...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const projectsData = {
        'burn-engine': {
            ...
            architecture: 'The Burn Engine uses a Program Derived Address (PDA) to manage burn operations. Each transaction triggers a CPI call to the SPL Token burn instruction. The protocol maintains a registry of burn events indexed by Helius webhooks.',
            miniTree: [
                {
                    icon: '🔧',
                    ...
                    future: 'Dynamic burn rates based on market conditions. Governance-controlled rate adjustments.'
                },
                {
                    icon: '📊',
                ...
                },
                {
                    ...
                    future: 'Continuous audit program. Bug bounty expansion. Regular re-audits after major updates.'
                },
                {
                    icon: '🌐',
        ...
        'burn-tracker': {
            ...
            title: 'Burn Tracker',
            status: 'live',
            overview: 'Real-time dashboard monitoring all burn transactions across the ecosystem. Aggregates data from on-chain events, calculates deflation metrics, and projects future supply based on historical burn rates.',
            features: [
                {
            ...
            architecture: 'Backend service subscribes to Helius webhooks for burn events. PostgreSQL stores historical data with time-series optimization. WebSocket server pushes updates to connected clients. Rate-limited public API exposes burn statistics.',
            miniTree: [
                {
                    icon: '📡',
        ...
        'token-launcher': {
            ...
            status: 'live',
            overview: 'One-click Solana SPL token deployment platform. Handles metadata creation via Metaplex, automatic liquidity pool setup on Raydium, and initial distribution configuration.',
            features: [
                {
            ...
            dependencies: 'Integrates with Burn Engine for burn-enabled tokens. Uses Metaplex for NFT metadata standard.',
            architecture: 'Frontend wizard collects token parameters. Backend validates and queues deployment. Smart contract creates mint, assigns authorities, and initializes metadata. Raydium pool creation is triggered post-deployment with configurable liquidity.',
            miniTree: [
                {
                    icon: '📝',
                    name: 'Token Creation',
        ...
        'oracle': {
            ...
            title: 'ASDF Oracle',
            status: 'building',
            overview: 'Proprietary oracle calculating the K-Metric, a composite health indicator for the ecosystem. Aggregates data from multiple on-chain and off-chain sources to provide actionable insights.',
            features: [
                {
            ...
            dependencies: 'Pulls data from Helius, Jupiter price feeds, Birdeye volume data, and internal burn metrics.',
            architecture: 'Data collectors run on schedule, fetching from various APIs. Python ML pipeline calculates K-Metric using weighted factors. InfluxDB stores time-series data. Grafana provides visualization. Webhook dispatcher handles alerts.',
            miniTree: [
                {
                    icon: '📊',
                    ...
                    future: 'Community-adjustable weights. Sub-metrics for specific aspects like liquidity health or holder concentration.'
                },
                {
                    icon: '🔗',
        ...
        'learn-platform': {
            ...
            status: 'live',
            overview: 'Gamified educational platform with 5 progressive levels. Users earn XP and badges by completing interactive tutorials and quizzes about the ASDF ecosystem.',
            features: [
                {
            ...
            dependencies: 'Standalone platform. Optional wallet connection for NFT badge minting (future).',
            architecture: 'Single-page application with vanilla JavaScript. LocalStorage persists progress across sessions. Service Worker enables offline access to completed lessons. Quiz validation happens client-side with server verification for badges.',
            miniTree: [
                {
                    icon: '📖',
        ...
        'rpc-monitor': {
            ...
            title: 'RPC Monitor',
            status: 'live',
            overview: 'Infrastructure health monitoring for all RPC endpoints. Tracks latency, uptime, and error rates with automatic alerting to Discord and Telegram.',
            features: [
                {
            ...
            architecture: 'Cron-based health checker pings RPC endpoints every 30 seconds. Results stored in Prometheus time-series database. Grafana dashboards visualize metrics. AlertManager triggers notifications on threshold breaches.',
            miniTree: [
                {
                    icon: '💓',
                    name: 'Health Checker',
        ...
        'holdex': {
            ...
            status: 'live',
            overview: 'Professional trading interface with TradingView charts, real-time order books, and technical analysis tools. Executes trades through Jupiter aggregator.',
            features: [
                {
            ...
            architecture: 'React frontend with TradingView widget integration. WebSocket feeds for real-time price updates. Jupiter SDK handles swap routing and execution. PostgreSQL stores user watchlists and alert configurations.',
            miniTree: [
                {
                    icon: '📈',
        ...
        'games-platform': {
            ...
            status: 'live',
            overview: 'Suite of browser-based mini-games with Solana wallet integration. Features weekly leaderboards, ticket system, and automatic reward distribution.',
            features: [
                {
            ...
            dependencies: 'Uses ASDF Oracle for reward calculations. Burn Engine integration for play-to-burn mechanics.',
            architecture: 'Microservices architecture with separate game engine and rewards service. Anti-cheat measures validate scores server-side. Redis stores session data and leaderboards. Smart contract handles reward distribution.',
            miniTree: [
                {
                    icon: '🎲',
                    name: '9 Mini-games',
        ...
        'community-hub': {
            ...
            title: 'Community Hub',
            status: 'building',
            overview: 'Centralized platform for community resources, events calendar, proposal system, and governance tools. Integrates with Discord for real-time sync.',
            features: [
                {
                    name: 'Event calendar',
            ...
            dependencies: 'Syncs with Discord server. Future integration with Ambassador Program for contribution tracking.',
            architecture: 'Next.js app with server-side rendering. Prisma ORM connects to PostgreSQL. Discord bot syncs announcements and events bidirectionally. Proposal system uses off-chain voting with on-chain verification.',
            miniTree: [
                {
                    icon: '📅',
                    name: 'Event Calendar',
        ...
        'deploy-pipeline': {
            ...
            status: 'building',
            overview: 'CI/CD automation for all ecosystem services. Handles testing, staging deployments, and production rollouts with blue-green strategy.',
            features: [
                {
            ...
            dependencies: 'Deploys all ecosystem services. Uses RPC Monitor for post-deploy health checks.',
            architecture: 'GitHub Actions trigger on PR merge. Docker builds pushed to registry. Terraform manages infrastructure. Kubernetes handles orchestration with blue-green deployment strategy. Post-deploy hooks verify service health.',
            miniTree: [
                {
                    icon: '🔄',
                    name: 'GitHub Actions',
        ...
        'ambassador-program': {
            ...
            status: 'planned',
            overview: 'Tiered contributor program with missions, ranks, and NFT badges. Tracks community contributions and automates reward distribution.',
            features: [
                {
                    name: 'Mission system',
            ...
            dependencies: 'Integrates with Community Hub for contribution data. Uses Metaplex for NFT badge minting.',
            architecture: 'Mission completion tracked via Discord bot and manual verification. Points accumulate toward rank progression. NFT badges minted on-chain for rank achievements. Reward distribution automated via smart contract.',
            miniTree: [
                {
                    icon: '📋',
                    name: 'Mission System',
        ...
        'content-factory': {
            ...
            status: 'planned',
            overview: 'Tools and templates for community content creators. Includes meme generator, post scheduler, and creator rewards program.',
            features: [
                {
            ...
            dependencies: 'Integrates with Ambassador Program for creator rewards. Discord bot for scheduled posting.',
            architecture: 'React app with canvas-based image editor. Templates stored in Cloudinary. Scheduler uses queue system for cross-platform posting. Engagement metrics tracked for creator rewards calculation.',
            miniTree: [
                {
                    icon: '🖼️',
                    name: 'Template Library',
        ...
        'security-audit': {
            ...
            status: 'planned',
            overview: 'Security scanning suite for smart contracts and APIs. Integrates Slither, Mythril, and OWASP ZAP with automated reporting.',
            features: [
                {
            ...
            dependencies: 'Scans all ecosystem smart contracts. Integrates with Deploy Pipeline for pre-deploy checks.',
            architecture: 'Python orchestration layer manages scanning tools. Slither and Mythril analyze Solana programs. OWASP ZAP handles API security testing. Reports generated in standardized format with severity scoring.',
            miniTree: [
                {
                    icon: '🔍',
        ...
        'forecast': {
            ...
            title: 'ASDForecast',
            status: 'live',
            overview: 'Prediction market platform allowing users to forecast prices and market events. Place predictions on token prices, ecosystem milestones, and earn rewards for accurate forecasts.',
            features: [
                {
            ...
            architecture: 'Prediction markets run on Solana smart contracts with PDA-based market accounts. Oracle integration for price resolution. React frontend for market creation and participation.',
            miniTree: [
                {
                    icon: '🎯',
                    name: 'Price Markets',
        ...
        'ignition': {
            ...
            title: 'Ignition',
            status: 'live',
            overview: 'Token launch platform and incubator for the ASDF ecosystem. Provides fair launch mechanisms, vesting schedules, and early access for community members.',
            features: [
                {
            ...
            architecture: 'Launch contracts manage contribution collection and token distribution. Vesting contracts handle scheduled unlocks. React frontend for project browsing and participation.',
            miniTree: [
                {
                    icon: '🚀',
    ...
    viewSwitchBtns.forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            viewSwitchBtns.forEach(function(b) { b.classList.remove('active'); });
            this.classList.add('active');

            viewSections.forEach(function(section) { section.classList.remove('active'); });
    ...
    skillPills.forEach(function(pill) {
        pill.addEventListener('click', function() {
            ...
            skillPills.forEach(function(p) { p.classList.remove('active'); });
            this.classList.add('active');
    ...
    const componentModalBackdrop = componentModal ? componentModal.querySelector('.modal-backdrop') : null;
    ...
    const codeLearningBackdrop = codeLearningModal ? codeLearningModal.querySelector('.modal-backdrop') : null;
    const codeLearningClose = codeLearningModal ? codeLearningModal.querySelector('.modal-close') : null;
    ...
    function generateConceptDiagram(component, project) {
        ...
        var diagramSvg = '<svg viewBox="0 0 500 250" style="width: 100%; max-width: 500px; height: auto;">' +
            '<defs>' +
        ...
        // Input box
        diagramSvg += '<rect x="10" y="100" width="100" height="50" rx="8" fill="rgba(74, 222, 128, 0.2)" stroke="#4ade80" stroke-width="2"/>' +
            '<text x="60" y="130" text-anchor="middle" fill="#4ade80" font-size="12" font-family="sans-serif">Input</text>';

        // Arrow 1
        diagramSvg += '<line x1="115" y1="125" x2="165" y2="125" stroke="#fb923c" stroke-width="2" marker-end="url(#arrowhead)"/>';

        // Process box (main component)
        diagramSvg += '<rect x="170" y="85" width="160" height="80" rx="12" fill="url(#flowGrad)" stroke="#ea580c" stroke-width="2"/>' +
            '<text x="250" y="115" text-anchor="middle" fill="white" font-size="11" font-family="sans-serif" font-weight="bold">' + icon + '</text>' +
            '<text x="250" y="135" text-anchor="middle" fill="white" font-size="13" font-family="sans-serif" font-weight="bold">' + name + '</text>' +
            '<text x="250" y="152" text-anchor="middle" fill="rgba(255,255,255,0.7)" font-size="10" font-family="sans-serif">' + status.toUpperCase() + '</text>';

        // Arrow 2
        diagramSvg += '<line x1="335" y1="125" x2="385" y2="125" stroke="#fb923c" stroke-width="2" marker-end="url(#arrowhead)"/>';

        // Output box
        diagramSvg += '<rect x="390" y="100" width="100" height="50" rx="8" fill="rgba(139, 92, 246, 0.2)" stroke="#8b5cf6" stroke-width="2"/>' +
            '<text x="440" y="130" text-anchor="middle" fill="#8b5cf6" font-size="12" font-family="sans-serif">Output</text>';

        // Sub-processes
        diagramSvg += '<rect x="170" y="185" width="70" height="35" rx="6" fill="rgba(251, 146, 60, 0.15)" stroke="#fb923c" stroke-width="1"/>' +
            '<text x="205" y="207" text-anchor="middle" fill="#fb923c" font-size="10" font-family="sans-serif">Parse</text>';
        diagramSvg += '<rect x="250" y="185" width="70" height="35" rx="6" fill="rgba(251, 146, 60, 0.15)" stroke="#fb923c" stroke-width="1"/>' +
        ...
        diagramSvg += '<line x1="205" y1="165" x2="205" y2="182" stroke="#fb923c" stroke-width="1" stroke-dasharray="4,2"/>';
        diagramSvg += '<line x1="285" y1="165" x2="285" y2="182" stroke="#fb923c" stroke-width="1" stroke-dasharray="4,2"/>';

        // Title
        diagramSvg += '<text x="250" y="30" text-anchor="middle" fill="#e2e8f0" font-size="14" font-family="sans-serif" font-weight="bold">Data Flow Architecture</text>' +
            '<text x="250" y="50" text-anchor="middle" fill="#94a3b8" font-size="11" font-family="sans-serif">' + sanitizeText(project.title) + '</text>';
    ...
    function generateCodeExamples(component, project) {
        ...
        if (lang === 'Rust') {
            code = '// ' + name + ' - ' + projectTitle + '\\n' +
                ...
                '        // Initialize the ' + name + ' state\\n' +
                '        let state = &mut ctx.accounts.state;\\n' +
                '        state.authority = ctx.accounts.authority.key();\\n' +
                '        state.is_active = true;\\n' +
                '        msg!("' + name + ' initialized successfully");\\n' +
                '        Ok(())\\n' +
                '    }\\n\\n' +
                '    pub fn process(ctx: Context<Process>, amount: u64) -> Result<()> {\\n' +
                '        // Process logic for ' + name + '\\n' +
                '        require!(amount > 0, ErrorCode::InvalidAmount);\\n' +
                '        \\n' +
                '        // Execute the core logic\\n' +
                '        let result = execute_' + name.toLowerCase() + '(amount)?;\\n' +
                '        \\n' +
                '        emit!(ProcessEvent {\\n' +
                '            amount,\\n' +
                '            result,\\n' +
        ...
        var tips = [
            {
                ...
                text: 'Before implementing ' + sanitizeText(component.name) + ', trace the data flow from input to output. Understanding how data transforms at each step prevents debugging headaches later.'
            },
            {
                icon: '⚠️',
    ...
    learningTabs.forEach(function(tab) {
        tab.addEventListener('click', function() {
            ...
            // Update tabs
            learningTabs.forEach(function(t) { t.classList.remove('active'); });
            this.classList.add('active');
    ...
    const realmObserver = new IntersectionObserver(function(entries) {
        entries.forEach(function(entry) {
            if (entry.isIntersecting) {
