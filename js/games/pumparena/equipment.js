...
function renderEquipmentPanel(container) {
    ...
    const statNames = {
        dev: 'Development', com: 'Community', mkt: 'Marketing',
        str: 'Strategy', cha: 'Charisma', lck: 'Luck'
    };

    container.innerHTML = `
        <div class="equipment-panel" style="background: #12121a; padding: 0; border-radius: 16px; overflow: hidden;">
            <!-- Header -->
            <div style="background: linear-gradient(135deg, #2e1a2e, #200d20); padding: 20px; border-bottom: 2px solid #a855f740;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <span style="font-size: 32px;">🛡️</span>
                    <div>
                        <h3 style="color: #a855f7; margin: 0; font-size: 20px;">Equipment</h3>
                        <div style="color: #c4b5fd; font-size: 12px;">Gear up for battle</div>
                    </div>
                </div>
            </div>
            ...
            <div style="padding: 20px; max-height: 65vh; overflow-y: auto;">
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px;">
                    ${Object.entries(EQUIPMENT_SLOTS).map(([slotId, slot]) => {
                        ...
                        return `
                            ...
                            " data-slot="${slotId}" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    ...
                    <div style="background: #1a1a24; border: 1px solid #333; border-radius: 12px; padding: 15px;">
                        ...
                        ${(bonuses.xpBonus > 0 || bonuses.critBonus > 0 || bonuses.defenseBonus > 0) ? `
                            <div style="margin-top: 10px; display: flex; flex-wrap: wrap; gap: 6px;">
                                ${bonuses.xpBonus > 0 ? `<span style="background: #22c55e20; color: #22c55e; padding: 4px 8px; border-radius: 4px; font-size: 11px;">+${bonuses.xpBonus}% XP</span>` : ''}
                                ${bonuses.critBonus > 0 ? `<span style="background: #ef444420; color: #ef4444; padding: 4px 8px; border-radius: 4px; font-size: 11px;">+${bonuses.critBonus}% Crit</span>` : ''}
                                ${bonuses.defenseBonus > 0 ? `<span style="background: #3b82f620; color: #3b82f6; padding: 4px 8px; border-radius: 4px; font-size: 11px;">+${bonuses.defenseBonus} Defense</span>` : ''}
                            </div>
                        ` : ''}
                    </div>
