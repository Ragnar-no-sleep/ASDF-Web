...
const Profile = {
  ...
  renderAchievements() {
    ...
    this.elements.achievementsGrid.innerHTML = this.data.achievements
      .map(
        a => `
  ...
  renderHistory() {
    ...
    this.elements.historyTable.innerHTML = this.data.history
      .slice(0, 10)
      .map(
        h => `
            <tr>
  ...
  renderInventory() {
    ...
    // Render items
    this.elements.inventoryGrid.innerHTML = items
      .map(item => {
