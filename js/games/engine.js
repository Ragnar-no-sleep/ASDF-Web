...
function startGame(gameId) {
    ...
    if (typeof AntiCheat !== 'undefined') {
        ...
        console.info(`Game session started: ${gameId} (${session.id.slice(0, 8)}...)`);
    }
...
function startHodlHero(gameId) {
    ...
    towerTypes.forEach(t => {
        ...
        btn.className = 'btn tower-shop-btn';
...
function startPumpArena(gameId) {
    const arena = document.getElementById(`arena-${gameId}`);
    const isBettingMode = false; // Betting mode disabled for legal compliance
...
function startRugPull(gameId) {
    ...
    function addWarning() {
        ...
        warningDiv.className = 'rug-warning';
        warningDiv.textContent = `${warning.icon} ${warning.text}`;
...
function startWhaleWatch(gameId) {
    ...
    state.patterns.forEach((icon, i) => {
        ...
        btn.dataset.index = i;
        btn.style.borderColor = state.colors[i];
