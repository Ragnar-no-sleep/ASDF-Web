...
const SolanaPayment = {
  ...
  async transferTokens(amount, destinationWallet = CONFIG.ESCROW_WALLET) {
    ...
    try {
      ...
      transaction.add(
        this.createTransferInstruction(fromTokenAccount, toTokenAccount, fromPubkey, rawAmount)
      );
  ...
  async getAssociatedTokenAddress(walletAddress, mintAddress) {
    const [address] = await solanaWeb3.PublicKey.findProgramAddress(
      [walletAddress.toBuffer(), this.getTokenProgramId().toBuffer(), mintAddress.toBuffer()],
