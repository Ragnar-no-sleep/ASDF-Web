...
const ShopItemCard = {
  ...
  render(item, options = {}) {
    ...
    const { showPrice = true, showFavorite = true, compact = false, selected = false } = options;
  ...
  renderPrice(item) {
    ...
    const originalPrice = hasDiscount
      ? item.originalPrice || Math.round(burnPrice / (1 - item.discount / 100))
      : null;
...
if (typeof window !== 'undefined') {
  ...
  window.ASDF.ShopItemCard = ShopItemCard;
  window.ShopItemCard = window.ASDF.ShopItemCard;
}
