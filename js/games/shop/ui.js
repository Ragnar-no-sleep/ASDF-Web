...
const ShopUI = {
  ...
  renderTabs() {
    ...
    if (tabsContainer) {
      tabsContainer.innerHTML = this.renderTabsHTML().replace(
        /<div class="shop-tabs">|<\/div>$/g,
        ''
      );
  ...
  renderItemCard(item) {
    ...
    const imageUrl =
      item.asset_url ||
      (window.ShopPlaceholders ? window.ShopPlaceholders.generate(item.layer, item.tier, 128) : '');
      ...
      for (const layer of ['aura', 'skin']) {
        ...
        if (itemId) {
          ...
          if (item && item.tier >= 5) {
            ...
            if (effectType) {
              const system = ShopParticles.createSystem(
                effectType,
                {
                  x: 0,
                  y: 0,
...
if (typeof window !== 'undefined') {
  ...
  window.ASDF.ShopUI = ShopUI;
  window.ShopUI = window.ASDF.ShopUI;
}
