 ...
 * Adapter Pattern for data access - JSON now, API later
 *
 * @version 2.0.0
 */
...
// ============================================
// INLINE PROJECT DATA
// (Extracted from build.js - will be moved to JSON file)
// ============================================

/**
 * Projects data - Full documentation for all ecosystem projects
 */
const projectsData = {
  'burn-engine': {
    id: 'burn-engine',
  ...
  'burn-tracker': {
    ...
    architecture: 'Backend subscribític to Helius webhooks. PostgreSQL stores historical data. WebSocket server pushes updates to clients.',
    miniTree: [
...
const DataAdapter = {
  /**
   * Set data source
   * @param {string} source - 'json' or 'api'
   */
  setSource(source) {
    if (source === DATA_SOURCES.JSON || source === DATA_SOURCES.API) {
   ...
   */
  async getProjects() {
    if (cache.projects && this.isCacheValid()) {
    ...
    if (currentSource === DATA_SOURCES.API) {
      ...
      // cache.projects = await response.json();
      throw new Error('API source not implemented yet');
    }

    // JSON source (inline data)
    cache.projects = projectsData;
   ...
   */
  async getBuilders() {
    if (cache.builders && this.isCacheValid()) {
   ...
   */
  async getSkills() {
    if (cache.skills && this.isCacheValid()) {
      return cache.skills;
    }
...
export { DataAdapter, projectsData, buildersData, skillsData };
export default DataAdapter;

// Global export for browser (non-module)
if (typeof window !== 'undefined') {
  window.DataAdapter = DataAdapter;
  window.BuildData = { projectsData, buildersData, skillsData };
}
