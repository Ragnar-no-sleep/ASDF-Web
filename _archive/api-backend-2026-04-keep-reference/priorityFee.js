...
function getFallbackEstimate(priorityLevel) {
  ...
  const avgFee =
    recentFees.reduce((sum, h) => sum + (h[priorityLevel] || h.medium), 0) / recentFees.length;
...
function getCongestionAnalysis() {
  ...
  const olderAvg =
    older.length > 0 ? older.reduce((sum, h) => sum + h.medium, 0) / older.length : recentAvg;
...
function getStats() {
  ...
  return {
    ...
    lastUpdate: lastEstimateTime ? new Date(lastEstimateTime).toISOString() : null,
  };
}
