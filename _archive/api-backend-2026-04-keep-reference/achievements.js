...
function getAchievementProgress(wallet) {
  ...
  return {
    ...
    byCategory,
    byRarity,
    totalXPFromAchievements: unlocked.reduce(
      (sum, id) => sum + (ACHIEVEMENTS[id]?.xpReward || 0),
      0
    ),
  };
}
...
function getDetailedAchievements(wallet) {
  ...
  return Object.values(ACHIEVEMENTS).map(achievement => ({
    ...
    unlockedAt: unlocked.has(achievement.id) ? getUnlockTime(wallet, achievement.id) : null,
...
function getAchievementMetrics() {
  const totalUnlocks = Array.from(userAchievements.values()).reduce(
  ...
  return {
    ...
    totalUnlocks,
    averageUnlocksPerPlayer:
      userAchievements.size > 0 ? (totalUnlocks / userAchievements.size).toFixed(2) : 0,
    rarestUnlocked,
  };
}
