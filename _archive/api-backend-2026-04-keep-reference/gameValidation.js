...
function startGameSession(wallet, gameType = 'default') {
  ...
  const token = crypto.createHmac('sha256', SESSION_SECRET).update(tokenData).digest('hex');
...
function checkForAnomalies(wallet, score, gameType) {
  ...
  const suspicious =
    zScore > 3 || percentile >= config.suspiciousThreshold * 100 || score > history.avgScore * 5;
