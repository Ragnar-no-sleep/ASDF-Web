...
function verifySignature(payload, signature, webhookType = 'burns') {
  const secret = HELIUS_WEBHOOK_CONFIG.secrets[webhookType] || HELIUS_WEBHOOK_CONFIG.secrets.burns;
...
function isBurnAddress(address) {
  ...
  return (
    lower === HELIUS_WEBHOOK_CONFIG.addresses.burn ||
...
function checkRateLimit(eventType) {
  ...
  const limit =
    HELIUS_WEBHOOK_CONFIG.rateLimits[eventType] || HELIUS_WEBHOOK_CONFIG.rateLimits.global;
...
async function handleWebhook(req, webhookType = 'burns') {
  ...
  // 1. Verify signature
  const signature = req.headers['x-helius-signature'] || req.headers['authorization'];
