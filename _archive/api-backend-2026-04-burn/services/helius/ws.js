...
class HeliusWebSocketManager extends EventEmitter {
  ...
  async subscribeAccount(publicKey, options = {}) {
    const { commitment = 'confirmed', encoding = 'base64' } = options;
  ...
  async subscribeSignature(signature, options = {}) {
    ...
    const subId = await this.sendRequest(SUBSCRIPTION_TYPES.SIGNATURE, [signature, { commitment }]);
  ...
  async subscribeLogs(filter, options = {}) {
    ...
    const subId = await this.sendRequest(SUBSCRIPTION_TYPES.LOGS, [filter, { commitment }]);
