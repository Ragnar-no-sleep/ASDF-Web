...
async function getWalletNFTs(wallet, options = {}) {
  const { page = 1, limit = DAS_CONFIG.defaultPageSize, collection = null } = options;
  ...
  if (collection) {
    items = items.filter(nft =>
      nft.grouping?.some(g => g.group_key === 'collection' && g.group_value === collection)
    );
  }
...
async function verifyCollectionOwnership(wallet, collectionAddress) {
  ...
  try {
    ...
    const collectionItems = (result.items || []).filter(nft =>
      nft.grouping?.some(g => g.group_key === 'collection' && g.group_value === collectionAddress)
    );
