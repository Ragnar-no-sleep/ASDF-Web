...
function getStorage() {
  if (!instance) {
    ...
    if (backend === BACKENDS.REDIS) {
      ...
      console.log(
        '[Storage] Redis URL:',
        process.env.REDIS_URL ? process.env.REDIS_URL.replace(/:([^@]+)@/, ':***@') : 'not set'
      );

      // Auto-connect if not already connecting
      if (!isConnecting) {
        isConnecting = true;
        connectionPromise = instance
          .connect()
          .then(() => {
      ...
      instance = new MemoryAdapter();
      console.log(
        '[Storage] Using in-memory storage (STORAGE_BACKEND=' +
          (process.env.STORAGE_BACKEND || 'not set') +
          ')'
      );
    }
  }
