...
const IDEMPOTENCY_CONFIG = {
  ...
  excludedPaths: ['/api/health', '/api/status', '/api/version'],
};
...
function generateBodyFingerprint(body) {
  ...
  return crypto
    .createHash(IDEMPOTENCY_CONFIG.hashAlgorithm)
    .update(normalized)
    .digest('hex')
    .substring(0, 16);
}
...
function validateIdempotencyKey(key) {
  ...
  if (key.length > IDEMPOTENCY_CONFIG.maxKeyLength) {
    return {
      valid: false,
      reason: `Key exceeds maximum length of ${IDEMPOTENCY_CONFIG.maxKeyLength}`,
    };
  }
...
function createCompositeKey(idempotencyKey, userId, method, path) {
  const components = [userId || 'anonymous', method.toUpperCase(), path, idempotencyKey];
...
function storeResponse(compositeKey, response, bodyFingerprint) {
  ...
  if (idempotencyStore.size >= IDEMPOTENCY_CONFIG.maxStoredResponses) {
    ...
    const entries = Array.from(idempotencyStore.entries()).sort(
...
function idempotencyMiddleware(options = {}) {
  ...
  return async (req, res, next) => {
    ...
    const compositeKey = createCompositeKey(idempotencyKey, userId, req.method, req.path);
