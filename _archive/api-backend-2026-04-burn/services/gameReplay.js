...
function recordAction(sessionId, action) {
  ...
  if (lastAction) {
    ...
    const minInterval =
      REPLAY_CONFIG.minActionInterval[replay.gameType] || REPLAY_CONFIG.minActionInterval.default;
...
function validateActionTiming(replay) {
  ...
  const gameType = replay.gameType;
  const minInterval =
    REPLAY_CONFIG.minActionInterval[gameType] || REPLAY_CONFIG.minActionInterval.default;
...
function validateScorePlausibility(replay, reportedScore) {
  ...
  if (reportedScore > maxPossibleScore * 1.5) {
    issues.push(
      `CRITICAL: Score ${reportedScore} exceeds maximum possible ${maxPossibleScore.toFixed(0)}`
    );
    penalty += 50;
...
function checkBurstLimit(replay, timestamp) {
  ...
  const maxActions =
    REPLAY_CONFIG.burstLimits.maxActions[replay.gameType] ||
...
function hashWallet(wallet) {
  return crypto.createHash('sha256').update(wallet).digest('hex').substring(0, 16);
}
...
function generateChecksum(replay) {
  ...
  return crypto.createHash(REPLAY_CONFIG.checksumAlgorithm).update(data).digest('hex');
}
...
function updateStats(replay) {
  ...
  stats.avgActionsPerGame = (stats.avgActionsPerGame * (n - 1) + replay.actions.length) / n;
  stats.avgGameDuration = (stats.avgGameDuration * (n - 1) + replay.duration) / n;
}
...
function cleanupOldReplays() {
  ...
  if (completedReplays.size > REPLAY_CONFIG.maxStoredReplays) {
    const sorted = Array.from(completedReplays.entries()).sort(
