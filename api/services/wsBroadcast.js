...
const CHANNEL_MAPPINGS = {
  // Burns channel
  burns: [EVENTS.BURN_INITIATED, EVENTS.BURN_CONFIRMED, EVENTS.BURN_FAILED],

  // Leaderboard channel
  leaderboard: [EVENTS.HIGH_SCORE, EVENTS.USER_TIER_CHANGED],

  // Games channel
  games: [EVENTS.GAME_STARTED, EVENTS.GAME_COMPLETED, EVENTS.HIGH_SCORE],

  // Global channel (system-wide)
  global: [EVENTS.WEBHOOK_RECEIVED, EVENTS.ERROR_OCCURRED],
...
function buildEventToChannelsMap() {
  ...
  if (BROADCAST_CONFIG.debug) {
    console.log(
      '[wsBroadcast] Event->Channel mappings:',
      Object.fromEntries(
        Array.from(EVENT_TO_CHANNELS.entries()).map(([k, v]) => [k, Array.from(v)])
      )
    );
  }
}
