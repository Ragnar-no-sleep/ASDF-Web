...
const ANTICHEAT_CONFIG = {
  ...
  weights: {
    ...
    sessionIntegrity: 0.1,
    environmentCheck: 0.1,
  },
...
function analyzeSession(context) {
  const { wallet, sessionId, gameType, score, duration, replay = null, clientInfo = {} } = context;
  ...
  factors.statisticalAnalysis = calculateStatisticalFactor(profile, gameType, score, duration);

  // 3. Behavior pattern factor
  factors.behaviorPattern = calculateBehaviorFactor(wallet, gameType, clientInfo);

  // 4. Historical reputation factor
  factors.historicalReputation = calculateReputationFactor(profile);

  // 5. Session integrity factor
  factors.sessionIntegrity = calculateSessionFactor(sessionId, duration, clientInfo);
...
function calculateReplayFactor(replay) {
  ...
  return {
    ...
    details: replay.issues?.length > 0 ? `Issues: ${replay.issues.join(', ')}` : 'Replay validated',
  };
}
...
function calculateReputationFactor(profile) {
  ...
  // Games played bonus
  const totalGames = Object.values(profile.gameHistory).reduce(
  ...
  if (profile.sanctionHistory.length > 0) {
    const recentSanctions = profile.sanctionHistory.filter(
...
function calculateEnvironmentFactor(clientInfo) {
  ...
  if (
    clientInfo.webglVendor === 'Brian Paul' ||
...
function calculateBanDuration(profile) {
  const banCount = profile.sanctionHistory.filter(
    s => s.type === SANCTION_TYPES.TEMPORARY_BAN
  ).length;
...
function applySanction(wallet, sanction) {
  ...
  const newSanction = {
    ...
    expiresAt: sanction.duration ? Date.now() + sanction.duration : null,
  };
...
function checkBan(wallet) {
  ...
  for (const sanction of sanctions) {
    if (
      sanction.type === SANCTION_TYPES.TEMPORARY_BAN ||
...
function updateProfile(profile, gameType, score, duration, sessionTrust, factors) {
  ...
  const oldWeight = 0.8;
  const newWeight = 0.2;
  profile.trustScore = Math.round(profile.trustScore * oldWeight + sessionTrust * newWeight);
...
function getProfile(wallet) {
  ...
  return {
    ...
    activeSanctions: (activeSanctions.get(wallet) || []).filter(
...
function getStats() {
  return {
    ...
    activeBans: Array.from(activeSanctions.values())
      ...
      .filter(
        s =>
          (s.type === SANCTION_TYPES.TEMPORARY_BAN || s.type === SANCTION_TYPES.PERMANENT_BAN) &&
