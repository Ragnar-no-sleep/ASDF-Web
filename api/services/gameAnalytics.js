...
function trackFunnelStep(userId, eventType, data) {
  ...
  for (const [funnelId, funnel] of funnels.entries()) {
    ...
    if (
      stepIndex === userProgress.currentStep + 1 ||
...
function getFunnelAnalysis(funnelId) {
  ...
  const conversions = funnel.steps.map((step, i) => {
    ...
    return {
      ...
      users: usersAtStep,
      conversionRate:
        totalStarted > 0 ? ((usersAtStep / totalStarted) * 100).toFixed(1) + '%' : '0%',
      dropoffRate:
        i > 0
  ...
  return {
    ...
    overallConversion:
      totalStarted > 0 ? ((funnel.completions / totalStarted) * 100).toFixed(1) + '%' : '0%',
    steps: conversions,
  };
}
...
function getExperimentResults(experimentId) {
  ...
  for (const [variant, data] of Object.entries(experiment.metrics)) {
    const conversionRate = data.users > 0 ? data.conversions / data.users : 0;

    results[variant] = {
      users: data.users,
      conversions: data.conversions,
      conversionRate: (conversionRate * 100).toFixed(2) + '%',
      avgValue: data.conversions > 0 ? (data.totalValue / data.conversions).toFixed(2) : '0',
    };
    ...
    if (variant === experiment.variants[0]) {
      ...
      results[variant].lift =
        (((conversionRate - controlRate) / controlRate) * 100).toFixed(1) + '%';
    }
  }
...
function updateSessionStats(sessionData) {
  ...
  if (sessionData.duration) {
    stats.avgSessionDuration = (stats.avgSessionDuration * (n - 1) + sessionData.duration) / n;
  }

  if (sessionData.gamesPlayed !== undefined) {
    stats.avgGamesPerSession = (stats.avgGamesPerSession * (n - 1) + sessionData.gamesPlayed) / n;
  }
}
