...
function createSession(userId, req, options = {}) {
  ...
  if (
    deviceRecord.sessionCount >= 3 &&
    deviceRecord.trustLevel === SESSION_CONFIG.trustLevels.NEW
  ) {
    deviceRecord.trustLevel = SESSION_CONFIG.trustLevels.RECOGNIZED;
  }
  if (
    deviceRecord.sessionCount >= 10 &&
    deviceRecord.trustLevel === SESSION_CONFIG.trustLevels.RECOGNIZED
  ) {
...
function revokeSession(sessionId, reason = 'manual') {
  ...
  console.log(
    `[SessionManager] Session revoked: ${sessionId.substring(0, 8)}... Reason: ${reason}`
  );
