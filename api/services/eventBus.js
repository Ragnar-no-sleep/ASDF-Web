...
async function publish(eventType, data = {}) {
  ...
  const sortedHandlers = Array.from(eventHandlers).sort((a, b) => b.priority - a.priority);
...
function getEventHistory(options = {}) {
  const { limit = 50, eventType = null, since = null } = options;
...
function getEventBusMetrics() {
  ...
  return {
    ...
    totalHandlers: Array.from(handlers.values()).reduce((sum, set) => sum + set.size, 0),
    handlerCounts,
