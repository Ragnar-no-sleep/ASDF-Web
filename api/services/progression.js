...
function awardXP(wallet, source, context = {}) {
  ...
  while (
    progress.currentXP >= getXPForNextLevel(progress.level) &&
...
function updateStreak(wallet) {
  ...
  if (progress.lastStreakUpdate === yesterday || progress.lastStreakUpdate === dayBefore) {
    // Grace period
    // Continue streak
    progress.streak = Math.min(progress.streak + 1, PROGRESSION_CONFIG.streaks.maxStreak);
