...
function search(query = {}) {
  ...
  const categories = category ? [category] : Object.keys(auditLogs);
...
function checkAlertConditions(event) {
  ...
  // Check thresholds
  if (
    event.type === EVENT_TYPES.LOGIN_FAILURE &&
  ...
  if (
    event.type === EVENT_TYPES.RATE_LIMITED &&
  ...
  if (
    event.type === EVENT_TYPES.SUSPICIOUS_ACTIVITY &&
...
function cleanup() {
  ...
  for (const [category, logs] of Object.entries(auditLogs)) {
    const retention = AUDIT_CONFIG.retention[category] || AUDIT_CONFIG.retention.operational;
...
// ============================================
// BACKWARD COMPAT — logAudit (was in leaderboard.js)
// ============================================

/**
 * Simple audit log (backward-compatible with leaderboard.logAudit).
 * Delegates to the structured log() function.
...
/**
 * Get recent audit events in the old format (backward-compatible).
 * @param {number} limit - Max events
...
module.exports = {
  ...
  // Legacy API (used by 38 services — was in leaderboard.js)
  logAudit,
  getAuditLog,
