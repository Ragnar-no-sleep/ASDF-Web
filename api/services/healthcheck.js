...
async function runAllChecks(type = null) {
  const checkList = Array.from(checks.values()).filter(c => !type || c.type === type);

  const results = await Promise.all(checkList.map(c => runCheck(c.name)));
