...
function handleDisconnect(code, reason) {
  ...
  if (reconnectAttempts < WS_CONFIG.maxReconnectAttempts) {
    ...
    setTimeout(
      () => {
        console.log(
          `[WS] Reconnecting (attempt ${reconnectAttempts}/${WS_CONFIG.maxReconnectAttempts})`
        );
        connect().catch(err => {
...
function maskUrl(url) {
  return url.replace(/api[_-]?key=([^&]+)/gi, 'api_key=***').replace(/\/([a-f0-9-]{36})/gi, '/***');
}
...
async function subscribeAccount(publicKey, callback, options = {}) {
  const { commitment = 'confirmed', encoding = 'jsonParsed', clientId = 'default' } = options;
...
async function subscribeSignature(signature, callback, options = {}) {
  const { commitment = 'confirmed', clientId = 'default' } = options;
...
async function subscribeSlot(callback, options = {}) {
  ...
  return createSubscription(SUBSCRIPTION_TYPES.SLOT, [], callback, clientId);
}
...
async function subscribeLogs(filter, callback, options = {}) {
  ...
  return createSubscription(SUBSCRIPTION_TYPES.LOGS, [filter, { commitment }], callback, clientId, {
    filter,
  });
}
...
function getSubscriptions(clientId = null) {
  ...
  return subs.map(s => ({
    ...
    lastNotification: s.lastNotification ? new Date(s.lastNotification).toISOString() : null,
    metadata: s.metadata,
  }));
}
