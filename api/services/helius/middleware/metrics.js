...
function recordWebSocket(event, data = {}) {
  switch (event) {
    ...
    case 'subscribe':
      ...
      break;
    case 'unsubscribe':
      gauges.websocket_active_subscriptions = Math.max(
        0,
        gauges.websocket_active_subscriptions - 1
      );
      break;
  }
}
...
function getPrometheusMetrics() {
  ...
  lines.push(
    '# HELP helius_circuit_breaker_state Circuit breaker state (0=closed, 0.5=half_open, 1=open)'
  );
  ...
  for (const [labels, buckets] of histograms.rpc_request_duration_ms) {
    ...
    for (const bound of METRICS_CONFIG.latencyBuckets) {
      lines.push(
        `helius_rpc_request_duration_ms_bucket{le="${bound}"${labelStr ? ',' + labels : ''}} ${buckets[bound]}`
      );
    }
    lines.push(
      `helius_rpc_request_duration_ms_bucket{le="+Inf"${labelStr ? ',' + labels : ''}} ${buckets['+Inf']}`
    );
...
function getJsonMetrics() {
  ...
  const cacheHitRate =
    cacheTotal > 0 ? ((counters.cache_hits_total / cacheTotal) * 100).toFixed(2) + '%' : 'N/A';
