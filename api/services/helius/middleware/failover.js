...
function selectEndpoint(type = ENDPOINT_TYPES.STANDARD, options = {}) {
  const { excludeIds = [], requireHealthy = true, preferStaked = false } = options;

  // Get all endpoints of this type
  const candidates = Array.from(endpoints.values()).filter(ep => {
  ...
  if (candidates.length === 0) {
    ...
    const fallback = Array.from(endpoints.values()).find(ep => {
      const health = healthStatus.get(ep.id);
...
async function executeEnhancedRequest(path, body = {}, options = {}) {
  const { timeout = RPC_CONFIG.requestTimeout, retries = RPC_CONFIG.maxRetries } = options;
...
async function checkEndpointHealth(id) {
  ...
  try {
    ...
    if (health.consecutiveSuccesses >= RPC_CONFIG.recoveryThreshold) {
      const wasUnhealthy =
        !primaryEndpoints.has(endpoint.type) || primaryEndpoints.get(endpoint.type) !== id;
...
function getEndpointStatus(id) {
  ...
  return {
    ...
    stats: {
      ...
      successRate:
        stats?.requests > 0 ? ((stats.successes / stats.requests) * 100).toFixed(2) + '%' : '100%',
    },
