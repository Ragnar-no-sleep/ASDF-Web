...
const ASDF_TOKEN_MINT =
  process.env.ASDF_TOKEN_MINT || '9zB5wRarXMj86MymwLumSKA1Dx35zPqqKfcZtK1Spump';
...
async function withRetry(fn, operation = 'RPC call') {
  ...
  for (let attempt = 0; attempt < RETRY_CONFIG.maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `[Helius] ${operation} failed (attempt ${attempt + 1}/${RETRY_CONFIG.maxRetries}): ${error.message}`
      );
...
async function getPriorityFeeEstimate(accountKeys = []) {
  ...
  try {
    ...
    const fee = Math.max(
      ...
      Math.min(
        data.result?.priorityFeeEstimate || PRIORITY_FEE_CONFIG.default,
        PRIORITY_FEE_CONFIG.max
      )
    );
...
async function getBatchTokenBalances(walletAddresses) {
  ...
  for (let i = 0; i < uncached.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.allSettled(batch.map(wallet => getTokenBalance(wallet)));
...
async function buildBurnTransaction(walletAddress, amount) {
  return withRetry(async () => {
    ...
    const priorityFee = await getPriorityFeeEstimate([mint.toBase58(), tokenAccount.toBase58()]);
  ...
  return withRetry(async () => {
    ...
    if (info.authority !== expectedWallet) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.warn(
          `[Security] Wrong authority: expected ${expectedWallet}, got ${info.authority}`
        );
      }
  ...
  return withRetry(async () => {
    ...
    const asdfBurns = transactions.filter(tx => {
      const transfer = tx.tokenTransfers?.find(
        t => t.mint === ASDF_TOKEN_MINT && t.tokenAmount < 0
      );
