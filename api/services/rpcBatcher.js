        ...
        setTimeout(() => {
            if (!request.completed) {
                ...
                request.callbacks.forEach(cb =>
                    cb.reject(new Error('Request timeout'))
                );
    ...
    try {
        ...
        try {
            ...
            for (const request of requests) {
                ...
                if (!result) {
                    request.callbacks.forEach(cb =>
                        cb.reject(new Error('No response for request'))
                    );
...
function sendTransaction(signedTransaction, options = {}) {
    return executeImmediate('sendTransaction', [signedTransaction, {
        encoding: 'base64',
        skipPreflight: false,
...
function simulateTransaction(transaction, options = {}) {
    return queueRequest('simulateTransaction', [transaction, {
        encoding: 'base64',
...
function getStats() {
    ...
    return {
        ...
        successRate: stats.totalBatches > 0
            ...
            : '100%',
        dedupeRate: stats.totalRequests > 0
